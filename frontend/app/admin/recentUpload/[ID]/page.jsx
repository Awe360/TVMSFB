...
const DetailPage = () => {
  ...
  const fetchMediaByID = async () => {
    try {
      const { data } = await axios.get(`https://tvmsb.onrender.com/api/media/find-media/${mediaID}`);
      setMediaData(data);
  ...
  const handleSchedule = async ({ mediaId, tvId,mediaUrl,public_id,mediaCategory, startDateTime, endDateTime, title, description ,schduledBy}) => {
    try {
      // Make API call
      const response = await axios.post('https://tvmsb.onrender.com/api/schedule/setSchedule',{mediaId, tvId,mediaUrl,public_id,mediaCategory, startDateTime, endDateTime, title, description });
  ...
  const deleteMedia = async ({id,public_id,mediaType}) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.post(`https://tvmsb.onrender.com/api/media/deleteMedia`,  { id,public_id,mediaType} );
  ...
  const displytoTVs=async({_id,tvId,mediaUrl,public_id,title,description,mediaCategory})=>{
    ...
    try {
     const res= await axios.post("https://tvmsb.onrender.com/api/media/display-to-tv", {
